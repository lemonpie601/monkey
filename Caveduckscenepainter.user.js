// ==UserScript==
// @name         Caveduck Scene Painter
// @namespace    caveduck-scene-painter
// @version      4.2.0
...
(function () {
    ...
    function getRoomId() {
        // caveduck.io: /talk/<uuid>
        const match = location.pathname.match(/\/talk\/([^/?#]+)/);
...
- 캐릭터 외형/작가태그/연도태그/Negative는 생성하지 않는다.`;
    ...
    function getDefaultRoomSettings() {
        return {
            ...
            caveduckImageMode: 'inline', // inline | side | both
            characters: [
                {
                    name: '',
                    appearanceTags: '',
    ...
    async function migrateHashKeysToStableIds() {
        ...
        const markdowns = Array.from(document.querySelectorAll('[id^="chat-message-"]'));
        const migrated = [];
        for (const el of markdowns) {
            const stableId = el.id || '';
            if (!stableId || records[stableId]) continue;
            const mdEl = el.querySelector('[class="[contain:paint]"]') || el.children?.[1] || el;
    ...
    function getMessageKey(markdown) {
        if (!markdown) return '';
        const group = markdown.matches?.('[id^="chat-message-"]') ? markdown : markdown.closest?.('[id^="chat-message-"]');
        const stableId = group?.id || '';
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            [id^='chat-message-'] .csp-generated-scene-image,
            [id^='chat-message-'] .csp-generated-scene-image,
            .css-peb4p4 .csp-generated-scene-image {
                margin-top: 12px !important;
                margin-bottom: 12px !important;
            }
            [id^='chat-message-'] p:has(+ .csp-generated-scene-image),
            [id^='chat-message-'] p:has(+ .csp-generated-scene-image),
            ...
            /* caveduck.io 배경 패널에 추가되는 🎨 삽화 컨트롤 */
            .csp-tab-button {
...
- 전신샷 → full body`;
    ...
    function isFooterLike(el) {
        // caveduck.io: 별도 footer 없음 - CSP 인라인 footer만 사용
        if (!el) return false;
        return el.classList?.contains('csp-inline-action-footer') === true;
    }
    ...
    function isComposerNode(node) {
        // caveduck.io: form.bg-background-500 또는 textarea
        const el = getMutationElement(node);
        if (!el || !el.closest) return false;
        if (el.closest('#chat-content-panel form')) return true;
        if (el.tagName === 'TEXTAREA' && el.closest('#chat-content-panel')) return true;
        return false;
    }
    function isChatListNode(node) {
        // caveduck.io: /talk/ 경로 외부는 채팅 외 페이지
        const el = getMutationElement(node);
        if (!el || !el.closest) return false;
        if (!location.pathname.includes('/talk/')) return true;
        return false;
    }
    function isMainMarkdown(el) {
        // caveduck.io: [id^="chat-message-"] 안의 [contain:paint] div
        if (!el || !el.classList) return false;
        if (el.closest('.csp-generated-scene-image')) return false;
        if (el.closest('#chat-content-panel form')) return false;
        // [contain:paint] 클래스를 가진 div가 콘텐츠 래퍼
        if (el.tagName === 'DIV' && el.className === '[contain:paint]') return true;
        // 또는 chat-message-* 직접 자식 div (두번째 자식)
        const msgEl = el.closest?.('[id^="chat-message-"]');
        if (msgEl && el === msgEl.children[1] && el.tagName === 'DIV') return true;
        return false;
    }
    ...
    function findPreviousMarkdown(footer) {
        ...
        while (cur && guard < 10) {
            ...
            const found = Array.from(cur.querySelectorAll?.('[id^="chat-message-"] .\\[contain\\:paint\\]') || []).find(isMainMarkdown);
        ...
        if (parent) {
            const candidates = Array.from(parent.querySelectorAll('[id^="chat-message-"] .\\[contain\\:paint\\]')).filter(isMainMarkdown);
    ...
    function getMessageGroupContainer(node) {
        // caveduck.io: [id^="chat-message-"]
        if (!node || !node.closest) return null;
        const el = getMutationElement(node) || (node.nodeType === 1 ? node : null);
        if (!el) return null;
        // 자기 자신이 chat-message-*인 경우
        if (el.matches?.('[id^="chat-message-"]')) return el;
        return el.closest?.('[id^="chat-message-"]') || null;
    }
    ...
    function getMessageGroupCandidates(scope = document) {
        ...
        if (root.matches?.('[id^="chat-message-"]')) set.add(root);
        root.querySelectorAll?.('[id^="chat-message-"]').forEach(group => set.add(group));
    ...
    function getDirectMarkdown(bubble) {
        // caveduck.io: [id^="chat-message-"] 내부 [contain:paint] div (두번째 자식)
        if (!bubble) return null;
        if (isMainMarkdown(bubble)) return bubble;
        const msgEl = bubble.matches?.('[id^="chat-message-"]') ? bubble : bubble.closest?.('[id^="chat-message-"]');
        const root = msgEl || bubble;
        // 두번째 자식이 콘텐츠 div
        const second = root.children?.[1];
        if (second && isMainMarkdown(second)) return second;
        // fallback: [contain:paint] 클래스
        const containPaint = Array.from(root.querySelectorAll?.('div') || []).find(d => d.className === '[contain:paint]');
        if (containPaint) return containPaint;
        return null;
    }
    function getFooter(bubble) {
        // caveduck.io: 별도 footer 없음, csp-inline-action-footer 반환
        if (!bubble) return null;
        if (isFooterLike(bubble)) return bubble;
        const msgEl = bubble.matches?.('[id^="chat-message-"]') ? bubble : bubble.closest?.('[id^="chat-message-"]');
        const root = msgEl || bubble;
        // CSP가 삽입한 inline footer
        const cspFooter = root.querySelector?.('.csp-inline-action-footer')
            || root.parentElement?.querySelector?.(':scope > .csp-inline-action-footer');
        return cspFooter || null;
    }
    ...
    function isUserMarkdown(markdown) {
        // caveduck.io: isAssistantMessageGroup으로 판별
        if (!markdown) return false;
        const msgEl = markdown.matches?.('[id^="chat-message-"]') ? markdown
            : markdown.closest?.('[id^="chat-message-"]');
        if (!msgEl) return false;
        return !isAssistantMessageGroup(msgEl);
    }
    ...
    function getMessageSideRole(node) {
        // caveduck.io: isAssistantMessageGroup으로 판별
        const el = getMutationElement(node);
        const msgEl = el?.matches?.('[id^="chat-message-"]') ? el : el?.closest?.('[id^="chat-message-"]');
        if (!msgEl) return '';
        return isAssistantMessageGroup(msgEl) ? 'assistant' : 'user';
    }

    function isAssistantMessageGroup(group) {
        // caveduck.io: AI 메시지는 첫 자식이 flex h-8 items-center justify-end (툴바 행)
        const el = getMutationElement(group);
        const msgEl = el?.matches?.('[id^="chat-message-"]') ? el : el?.closest?.('[id^="chat-message-"]');
        if (!msgEl) return false;
        if (isComposerNode(group)) return false;
        const firstChild = msgEl.children?.[0];
        if (!firstChild) return false;
        const hasToolbar = firstChild.tagName === 'DIV' &&
            firstChild.classList?.contains('flex') &&
            firstChild.classList?.contains('h-8') &&
            firstChild.classList?.contains('justify-end');
    ...
    function cleanupNonAssistantMessageButtons() {
        document.querySelectorAll('.csp-inline-action-footer').forEach(footer => {
            const markdown = findPreviousMarkdown(footer)
                || footer.parentElement?.querySelector?.(':scope > [class="[contain:paint]"]')
                || null;
        ...
        document.querySelectorAll('.csp-message-generate-btn, .csp-message-speed-btn').forEach(btn => {
            ...
            const markdown = group ? getDirectMarkdown(group) : (btn.closest('[id^="chat-message-"]') || findPreviousMarkdown(btn.closest('.csp-inline-action-footer')));
    ...
    function isUserBubble(bubble) {
        ...
        const group = (bubble.matches?.('[id^="chat-message-"]') ? bubble : null) || getMessageGroupContainer(bubble);
        if (group) {
            if (isAssistantMessageGroup(group)) return false;
            // caveduck: bg-surface_chat_secondary 없음
    ...
    function getAssistantBubbles(scope = document) {
        // caveduck.io: [id^="chat-message-"] + AI 툴바
        const set = new Set();
    ...
    function getAllChatBubbles() {
        ...
        if (!set.size) {
            Array.from(document.querySelectorAll('[id^="chat-message-"]'))
    ...
    function getInsertableContentBlocks(markdown) {
        ...
        // caveduck.io: [contain:paint] div 안 SPAN들이 단락 역할
        // 텍스트 있는 SPAN만 블록으로 사용
    ...
    function insertSceneImageIntoMarkdown(markdown, imageUrl, paragraphIndex, options = {}) {
        ...
        // caveduck.io: 이미지 삽입 모드 결정
        // displayMode: 'inline'(본문 삽입) | 'side'(#chat-background-panel 교체) | 'both'
        const room = getRoomSettings();
        const displayMode = room?.caveduckImageMode || 'inline';

        // ── 사이드 패널 표시 (배경 패널 이미지 교체) ──────────────────────
        if (displayMode === 'side' || displayMode === 'both') {
            showImageInBackgroundPanel(imageUrl, options);
        }
        if (displayMode === 'side') {
            // 본문 삽입 없이 사이드만
            return { ok: true, index: 0, target: markdown, box: null, historyRow: null, sideOnly: true };
        }
    ...
    // caveduck.io: #chat-background-panel의 img를 교체하여 사이드에 이미지 표시
    function showImageInBackgroundPanel(imageUrl, options = {}) {
        const panel = document.getElementById('chat-background-panel');
        if (!panel) return;
        const img = panel.querySelector('img');
        if (!img) return;
        // 원본 src 백업 (처음 한 번만)
        if (!img.dataset.cspOrigSrc) img.dataset.cspOrigSrc = img.src;
        img.src = imageUrl;
        img.style.objectFit = 'contain';
        img.style.objectPosition = 'top';
        // CSP 사이드 이미지임을 표시
        img.dataset.cspSideImage = '1';
        if (options.messageKey) img.dataset.cspMessageKey = options.messageKey;
    }

    // caveduck.io: 사이드 패널 이미지 원상복구
    function restoreBackgroundPanel() {
        const panel = document.getElementById('chat-background-panel');
        if (!panel) return;
        const img = panel.querySelector('img[data-csp-side-image]');
        if (!img) return;
        if (img.dataset.cspOrigSrc) img.src = img.dataset.cspOrigSrc;
        delete img.dataset.cspSideImage;
        delete img.dataset.cspMessageKey;
    }
    ...
    function openGalleryModal() {
        ...
        const drawerBackdropG = Array.from(document.querySelectorAll('div')).find(d => d.className === 'fixed inset-0 bg-black/40');
        if (drawerBackdropG) drawerBackdropG.style.pointerEvents = 'none';
        const close = () => { overlay.remove(); };
        new MutationObserver((_, obs) => {
            if (!overlay.isConnected) {
                const bd = Array.from(document.querySelectorAll('div')).find(d => d.className === 'fixed inset-0 bg-black/40');
...
[이미 선택된 문단 인덱스: ${[...usedParagraphs].join(', ')} — 이 인덱스는 사용하지 마.]`
    ...
    function openSettingsModal() {
        ...
        overlay.innerHTML = `
            <div class="csp-modal" role="dialog" aria-modal="true">
                <div style="display:flex;align-items:center;justify-content:space-between;margin:0 0 6px;">
                    <h2 style="margin:0;">🎨 Caveduck Scene Painter 설정</h2>
                    <button id="csp-close-top" type="button" style="background:none;border:none;cursor:pointer;font-size:22px;line-height:1;color:inherit;opacity:0.5;padding:4px 8px;border-radius:6px;" title="닫기">✕</button>
                </div>
                ...
                <div class="csp-section">
                    <div class="csp-section-title" style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;" data-csp-section-toggle="prompt-nai-settings">
                        <span>공통 NAI 생성 설정</span>
                        <button class="csp-btn csp-btn-small" type="button" style="padding:2px 8px;font-size:11px;" data-csp-section-arrow="prompt-nai-settings">▲</button>
                    </div>
                ...
                <div class="csp-section">
                    <div class="csp-section-title">🖼️ 케이브덕 이미지 표시 방식</div>
                    <div class="csp-grid">
                        <div class="csp-field">
                            <label>생성된 이미지를 어디에 표시할지 선택</label>
                            <select id="csp-caveduck-image-mode">
                                <option value="inline" ${(room.caveduckImageMode||'inline')==='inline'?'selected':''}>📝 본문 삽입 (채팅 메시지 안에)</option>
                                <option value="side" ${(room.caveduckImageMode||'inline')==='side'?'selected':''}>🖼️ 사이드 패널 (캐릭터 이미지 위치)</option>
                                <option value="both" ${(room.caveduckImageMode||'inline')==='both'?'selected':''}>✨ 둘 다 (본문 + 사이드)</option>
                            </select>
                        </div>
                    </div>
                </div>
        ...
        overlay.querySelector('#csp-prompt-preset-save')?.addEventListener('click', () => {
            ...
            const preset = { name, basePositive: overlay.querySelector('#csp-base-positive').value.trim(), baseNegative: overlay.querySelector('#csp-base-negative').value.trim(), ucPreset: Number(overlay.querySelector('#csp-default-uc-preset')?.value || 0), naiSettings: {
                ...
                nSamples: 1, smea: false, dyn: false,
            ...
            try { saveGlobalSettings(collectGlobal()); } catch(e) {}
        ...
        overlay.querySelector('#csp-prompt-preset-load')?.addEventListener('click', () => {
            ...
            if (preset.naiSettings) {
                const ns = preset.naiSettings;
                const orientEl = overlay.querySelector('#csp-default-orientation');
                const wEl = overlay.querySelector('#csp-default-width');
                const hEl = overlay.querySelector('#csp-default-height');
                const wViewEl = overlay.querySelector('#csp-default-width-view');
                const hViewEl = overlay.querySelector('#csp-default-height-view');
                if (orientEl) { orientEl.value = ns.orientationPreset || 'portrait'; applyOrientationPreset(orientEl.value, wEl, hEl, wViewEl, hViewEl); }
                const stepsRange = overlay.querySelector('#csp-default-steps-range');
                const stepsInput = overlay.querySelector('#csp-default-steps');
                const stepsVal = overlay.querySelector('#csp-default-steps-value');
                if (stepsRange) stepsRange.value = ns.steps ?? 28;
                if (stepsInput) stepsInput.value = ns.steps ?? 28;
                if (stepsVal) stepsVal.textContent = ns.steps ?? 28;
                const scaleRange = overlay.querySelector('#csp-default-scale-range');
                const scaleInput = overlay.querySelector('#csp-default-scale');
                const scaleVal = overlay.querySelector('#csp-default-scale-value');
                if (scaleRange) scaleRange.value = ns.scale ?? 6.5;
                if (scaleInput) scaleInput.value = ns.scale ?? 6.5;
                if (scaleVal) scaleVal.textContent = Number(ns.scale ?? 6.5).toFixed(1);
                const grRange = overlay.querySelector('#csp-default-guidance-rescale-range');
                const grInput = overlay.querySelector('#csp-default-guidance-rescale');
                const grVal = overlay.querySelector('#csp-default-guidance-rescale-value');
                if (grRange) grRange.value = ns.guidanceRescale ?? 0.3;
                if (grInput) grInput.value = ns.guidanceRescale ?? 0.3;
                if (grVal) grVal.textContent = Number(ns.guidanceRescale ?? 0.3).toFixed(2);
                const seedEl = overlay.querySelector('#csp-default-seed');
                if (seedEl) seedEl.value = ns.seed ?? '';
                const samplerEl = overlay.querySelector('#csp-default-sampler');
                if (samplerEl) samplerEl.value = ns.sampler || 'k_euler_ancestral';
                const noiseEl = overlay.querySelector('#csp-default-noise-schedule');
                if (noiseEl) noiseEl.value = ns.noiseSchedule || 'karras';
            }
        ...
        overlay.querySelector('#csp-gemini-preset-save')?.addEventListener('click', () => {
            ...
            if (!name) { showToast('⚠️ 프리셋 이름을 입력해줘.'); return; }
            const preset = { name, naiPromptGuide: overlay.querySelector('#csp-nai-prompt-guide').value.trim(), geminiInstruction: overlay.querySelector('#csp-gemini-instruction').value.trim(), naiSettings: {
                orientationPreset: overlay.querySelector('#csp-default-orientation').value,
                width: Number(overlay.querySelector('#csp-default-width').value || 832),
                height: Number(overlay.querySelector('#csp-default-height').value || 1216),
                steps: Number(overlay.querySelector('#csp-default-steps').value || 28),
        ...
        overlay.querySelector('#csp-gemini-preset-load')?.addEventListener('click', () => {
            ...
            if (preset.naiSettings) {
                ...
                const noiseEl = overlay.querySelector('#csp-default-noise-schedule');
                if (noiseEl) noiseEl.value = ns.noiseSchedule || 'karras';
                const ucEl = overlay.querySelector('#csp-default-uc-preset');
                if (ucEl) ucEl.value = ns.ucPreset ?? 0;
            }
        ...
        // ── 프롬프트 탭 섹션 접기 ──────────────────────────────────────
        ['prompt-fixed', 'prompt-gemini', 'prompt-nai-settings'].forEach(key => {
        ...
        function collectRoom() {
            ...
            const modeEl = overlay.querySelector('#csp-caveduck-image-mode');
            return {
                caveduckImageMode: modeEl?.value || 'inline',
                characters: characters.length ? characters : [{ name: '', appearanceTags: '', outfitTags: '', tags: '', uc: '' }]
            };
        }

        // overlay 배경은 pointer-events:none이라 클릭 안됨
        // ESC 키로만 닫기 가능 (배경클릭은 드로어 닫힘 유발하므로 제거)

        // ESC 키로 닫기 (stopPropagation으로 드로어 ESC 핸들러 방지)
        ...
        document.addEventListener('keydown', onKeyDown, true); // capture phase로 먼저 가로챔
        overlay.addEventListener('remove', () => document.removeEventListener('keydown', onKeyDown, true));

        overlay.querySelector('#csp-close').onclick = () => { overlay.remove(); setTimeout(reopenCaveduckDrawer, 50); };
        overlay.querySelector('#csp-close-top').onclick = () => { overlay.remove(); setTimeout(reopenCaveduckDrawer, 50); };
        ...
        // 드로어 backdrop 클릭 차단 (CSP 모달 열려있는 동안)
        const drawerBackdrop = Array.from(document.querySelectorAll('div')).find(d => d.className === 'fixed inset-0 bg-black/40');
        ...
        // 닫힐 때 복원 (MutationObserver)
        new MutationObserver((_, obs) => {
            if (!overlay.isConnected) { restoreBackdrop(); obs.disconnect(); }
        }).observe(document.body, { childList: true });
    ...
    function showQuickPanel() {
        ...
        // 기본 위치: 드로어가 오른쪽에 있으므로 중앙 좌측에 배치
        const defaultLeft = Math.min(20, window.innerWidth - 280);
        panel.style.top = (savedPos?.top ?? 120) + 'px';
        panel.style.left = (savedPos?.left ?? defaultLeft) + 'px';
        ...
        const _startDrag = (clientX, clientY) => {
            dragging = true;
            ox = clientX - panel.getBoundingClientRect().left;
            oy = clientY - panel.getBoundingClientRect().top;
        };
        const _moveDrag = (clientX, clientY) => {
            if (!dragging) return;
            const x = Math.max(0, Math.min(window.innerWidth - panel.offsetWidth - 540, clientX - ox));
            const y = Math.max(0, Math.min(window.innerHeight - panel.offsetHeight, clientY - oy));
        ...
        const _onMove = (e) => { _moveDrag(e.clientX, e.clientY); };
    ...
    function findCaveduckDrawerNav() {
        // Setting 버튼으로 열리는 오른쪽 드로어 안의 nav 반환
        const drawer = Array.from(document.querySelectorAll('div')).find(d =>
            d.classList?.contains('fixed') &&
            d.classList?.contains('right-0') &&
            d.classList?.contains('bg-background-700') &&
            d.getBoundingClientRect().width > 200
        );
        if (!drawer) return null;
        return drawer.querySelector('nav') || null;
    }

    function findViewerTabContent() {
        // caveduck.io: 오른쪽 대화설정 드로어의 nav 끝에 CSP 컨테이너 삽입
        let cspContainer = document.getElementById('csp-caveduck-container');
        if (cspContainer && cspContainer.isConnected) return cspContainer;

        const nav = findCaveduckDrawerNav();
        if (!nav) return null;

        cspContainer = document.createElement('div');
        cspContainer.id = 'csp-caveduck-container';
        nav.appendChild(cspContainer);
        return cspContainer;
    }
    ...
    function syncCspRowVisibility() {
        ...
        if (painterRow) painterRow.style.display = '';
        updateGalleryRowCount();
    }

    function reopenCaveduckDrawer() {
        // 드로어가 닫혀있으면 Setting 버튼 클릭으로 재개
        if (!findCaveduckDrawerNav()) {
            const btn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.trim() === 'Setting');
            btn?.click();
        }
    }

    function cspIcon(paths) {
        return `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">${paths}</svg>`;
    }

    function makeCaveduckNavItem({ id, icon, label, onClick }) {
        const btn = document.createElement('button');
        if (id) btn.id = id;
        btn.type = 'button';
        btn.className = 'inline-flex items-center cursor-pointer shrink-0 gap-2 whitespace-nowrap rounded-sm font-medium transition-colors focus-visible:outline-none w-full justify-between py-2.5 px-0 text-sm';
        btn.innerHTML = `
            <span class="flex items-center gap-2">${icon}<span>${label}</span></span>
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="opacity:0.4;flex-shrink:0;"><path d="m9 18 6-6-6-6"/></svg>
        `;
        if (onClick) btn.addEventListener('click', onClick);
        return btn;
    }

    function makeCaveduckToggleItem({ id, icon, label, checked, onToggle }) {
        const row = document.createElement('div');
        if (id) row.id = id;
        row.style.cssText = 'display:flex;align-items:center;justify-content:space-between;padding:10px 0;';
        const left = document.createElement('span');
        left.style.cssText = 'display:flex;align-items:center;gap:8px;font-size:14px;font-weight:500;';
        ...
        toggle.className = 'peer focus-visible:ring-ring focus-visible:ring-offset-background data-[state=checked]:bg-primary data-[state=unchecked]:bg-dgray-800 light:data-[state=unchecked]:bg-dgray-300 inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent shadow-sm transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50';
        const thumb = document.createElement('span');
        thumb.setAttribute('data-state', checked ? 'checked' : 'unchecked');
        thumb.className = 'pointer-events-none block h-4 w-4 rounded-full bg-background shadow-sm ring-0 transition-transform data-[state=checked]:translate-x-4 data-[state=unchecked]:translate-x-0';
        toggle.appendChild(thumb);
        row.appendChild(left);
        row.appendChild(toggle);
        function setVal(val) {
            const st = val ? 'checked' : 'unchecked';
            toggle.setAttribute('data-state', st);
            toggle.setAttribute('aria-checked', String(val));
            thumb.setAttribute('data-state', st);
            if (onToggle) onToggle(val);
        }
        toggle.addEventListener('click', () => setVal(toggle.getAttribute('aria-checked') !== 'true'));
        row._setVal = setVal;
        return row;
    }
    ...
    function injectScenePainterRow() {
        ...
        section.id = 'csp-scene-painter-row';

        const header = document.createElement('h3');
        header.className = 'text-md mb-2 font-semibold';
        header.style.marginTop = '8px';
        header.textContent = 'AI 삽화';
        section.appendChild(header);

        // 1) AI 삽화 생성 토글
        const paintIcon = cspIcon('<path d="M18.37 2.63 14 7l-1.59-1.59a2 2 0 0 0-2.82 0L8 7l9 9 1.59-1.58a2 2 0 0 0 0-2.82L17 10l4.37-4.37a2.12 2.12 0 1 0-3-3Z"/><path d="M9 8c-2 3-4 3.5-7 4l8 10c2-1 6-5 6-7"/><path d="M14.5 17.5 4.5 15"/>');
        const toggleRow = makeCaveduckToggleItem({
            id: 'csp-caveduck-toggle',
            icon: paintIcon,
            label: 'AI 삽화 생성',
            checked: isEnabled(),
            onToggle: (val) => {
                setEnabled(val);
                applySceneVisibilityState(val);
                if (!val) document.querySelectorAll('.csp-message-generate-btn').forEach(b => b.remove());
                else scheduleInject();
                showToast(val ? '🎨 AI 삽화 생성 ON' : '⏸️ AI 삽화 생성 OFF');
            }
        });
        section.appendChild(toggleRow);

        // 2) 삽화 갤러리
        const galleryIcon = cspIcon('<rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/>');
        section.appendChild(makeCaveduckNavItem({
            id: 'csp-scene-gallery-row',
            icon: galleryIcon,
            label: '삽화 갤러리',
        ...
        // 3) 퀵 패널
        const zapIcon = cspIcon('<path d="M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z"/>');
        section.appendChild(makeCaveduckNavItem({
            id: 'csp-quick-panel-row',
            icon: zapIcon,
            label: '퀵 패널',
            onClick: () => { _quickPanelVisible = !_quickPanelVisible; if (_quickPanelVisible) showQuickPanel(); else hideQuickPanel(); }
        }));

        // 4) 설정 — 채팅방 설정 섹션과 동일한 위치에
        const settingsIcon = cspIcon('<path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/>');
        section.appendChild(makeCaveduckNavItem({
            id: 'csp-caveduck-settings-btn',
            icon: settingsIcon,
            label: 'Scene Painter 설정',
    ...
    function mutationTouchesMenuArea(mutation) {
        ...
        return nodes.some(node => {
            ...
            if (node.id === 'chat-background-panel') return true;
            if (node.id === 'chat-horizontal-group') return true;
            if (node.classList?.contains('fixed') && node.classList?.contains('right-0') &&
                node.classList?.contains('bg-background-700')) return true;
            if (node.tagName === 'NAV' && node.closest?.('[class*="fixed"][class*="right-0"]')) return true;
            return false;
        });
    }
    ...
    function mutationTouchesAssistantMessage(mutation) {
        ...
        return nodes.some(node => {
            ...
            if (node.matches?.('[id^="chat-message-"]')) return true;
            if (node.closest?.('[id^="chat-message-"]')) return true;
            if (node.id === 'chat-content-panel') return true;
            return false;
        });
    }
    ...
    function setupObserver() {
        const observer = new MutationObserver((mutations) => {
            ...
            for (const mutation of mutations) {
                ...
                if (mutationTouchesMenuArea(mutation)) needsMenu = true;
                if (mutationTouchesAssistantMessage(mutation)) needsMessage = true;
                const el = getMutationElement(mutation.target);
                if (el?.id === 'chat-main' || el?.id === 'chat-horizontal-group') needsAll = true;
            }
            if (needsAll) { scheduleInject(); return; }
            if (needsMenu) scheduleMenuInject();
            if (needsMessage) scheduleMessageInject();
        });
    ...
    function init() {
        ...
        injectAll();
        setupObserver();
        // CSP 모달이 닫힐 때마다 드로어 재오픈
        new MutationObserver((mutations) => {
            for (const m of mutations) {
                for (const node of m.removedNodes) {
                    if (node.nodeType === 1 && node.classList?.contains('csp-overlay')) {
                        setTimeout(reopenCaveduckDrawer, 80);
                        return;
                    }
                }
            }
